...
领取任务会附带提交必填项与函数详情模板。准备失败时，工具按节点和字段返回当前值、期望类型及修复建议；可独立检查的缺字段、任务完成状态和源码缺行尽量一次列出。字段修复可复用当前连接已经读过的材料；确实缺少源码时返回具体缺行。校验不会自动补写语义数组或把未完成任务标成完成。
...
选中文件、模块、节点或任务时，相机会平滑移动到对应节点或节点组，按详情面板展开后的画布尺寸居中，尽量保持当前缩放；目标较大时适当缩小。窄屏会收起遮挡画布的浮层，选中内容的详情仍可通过顶部“详情”打开。拖动、滚轮缩放、再次选择或切换视图会中止旧的定位动画。节点位置和锁定状态保持不变；系统设置为减少动态效果时直接定位。
...
node --test tests/canvas.test.cjs tests/topology.test.cjs tests/workflow.test.cjs tests/reader.test.cjs tests/camera.test.cjs
```
