...
## Scale-path storage and maintenance

Current storage omits full text from files.document and source_text from nodes.document. The legacy structure API reconstructs symbols/sites from relations; automatic legacy-cache writes are disabled unless init/sync opts into legacy_cache=true. Explicit old index builds may still write their compatibility cache. Previous skeleton formats require init into a NEW output directory: do not overwrite the old map or imply its semantic history was migrated. Ordinary sync only supports the current format; reindex is not a storage migration.

FTS updates use an indexed node-to-rowid map and touch only the batch or changed/invalidated nodes. Use blueprint_reindex for an explicit full repair. Call-site indexes select changed files plus sites naming changed candidate symbols. Same-line repeated calls from the same source to the same target are collapsed by the edge primary key. Call degree is materialized from those stored edges. repo_map path selects a normalized directory prefix using an indexed range.

Claim reads and verifies source before acquiring the write reservation, then rechecks the skeleton generation and lease/task state. A concurrent change retries selection. Source excerpts and hashes still must match on submission. Independent 100K mypy measurements have not been performed during this implementation phase.
